...
public class JMXTimeoutManager implements TimeoutManager, TimeoutManagerMXBean {
	...
	TreeMultiset<DecoratedTimeout> defensive = TreeMultiset.create(keepAliveTimeouts.headMultiset(
		new DecoratedTimeout(null, new Timeout(now, AsyncCallback.nopCb)), BoundType.CLOSED));
	...
	return keepAliveTimeouts.isEmpty() ? Long.MAX_VALUE : Math.max(1,
		keepAliveTimeouts.iterator().next().timeout.getTimeout() - now);
    }
